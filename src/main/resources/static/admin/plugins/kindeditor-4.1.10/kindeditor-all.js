        ...
        init: function (options) {
            ...
            var settings = {
                ...
                upload_success_handler: function (file, serverData) {
                    ...
                    if (data.code != 200) {
                        showError(itemDiv, K.DEBUG ? data.msg : self.options.errorMessage);
                        return;
                    }
                    file.url = data.map.url;
        ...
        self.plugin.multiImageDialog({
            clickFn: function (urlList) {
                ...
                K.each(urlList, function (i, data) {
                    ...
                    self.exec('insertimage', data.map.url, data.title, data.width, data.height, data.border, data.align);
                });
                // Bugfix: [Firefox] 上传图片后，总是出现正在加载的样式，需要延迟执行hideDialog
                setTimeout(function () {
                    self.hideDialog().focus();
